...
function Profile(props) {
  ...
  const [internalProfileUrl, setInternalProfileUrl] = useState(props.userDet.get("profileUrl"))
  ...
  useEffect(() => {
    if ((props.userDet.get("bio") !== internalBio) || (props.userDet.get("profileUrl") !== internalProfileUrl)) {
      setAllowUpdate(true)
    } else {
      ...
      <ProfileInner>
        <ProfileInnerLeft>
          <div className="main-img">
            <img src={`/images/profile/${internalProfileUrl}`} alt="main-profile" />

          </div>
          <div className="img-option">
            <p>Change Your <span className='active-clr'>Profile Picture</span></p>
            {
              //Image click event and the click event carries an image value that value changes in the profile
              profileImgs.map(imgSrc => {
                return (

                  <button onClick={() => setInternalProfileUrl(imgSrc)}>
                    {console.log(typeof (imgSrc), "imgSrc")}
        ...
        <ProfileInnerRight>
          ...
          <div className="bioEditor">
            <div className='edit-elm' ref={bioEditRef} >
              <ReactQuill modules={modules?.module} theme="snow" value={internalBio} onChange={(content) => setInternalBio(content)} placeholder='Write here...' />
            </div>
            <button className={`btn-style ${!allowUpdate ? "disable" : ""}`} onClick={handleEditor}>Update</button>
          </div>
